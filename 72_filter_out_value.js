...
users.map(function (users, index) {
  if (users.address === "kathmandu") userKtm.push(users);
});
...
let userKtmm = users.filter(function (users, index) {
  return users.address === "kathmandu";
}); // this is the most important syntex for the
console.log({ happy: userKtm }); // takeout the similar object takeout

let userBikash = users.filter(function (users, index) {
  return users.name === "bikash";
});
console.log( {new: userBikash} );
